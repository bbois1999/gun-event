...
export function useCheckImagePost<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; title?: string; content?: string; published?: boolean; image?: string; authorId?: string; eventId?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
